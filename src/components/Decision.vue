	...
	export default {
		...
		methods: {
			...
			confirmOldIGN() {
				...
				console.log("button text: ", this.confirmButtonText);
				this.confirmButtonText = 'Confirmed!';
				console.log("button text: ", this.confirmButtonText);
			},
			confirmNewPlayer() {
				axios.post('/api/decide', {
...
<template>
	...
	<div v-if="this.decisionState == 'ConfirmOldIGN'" class="decision">
		...
		<BigButton @click="confirmOldIGN()" :label="confirmButtonText"/>
	</div>
	...
	<div v-if="this.decisionState == 'ConfirmNewPlayer'" class="decision">
		...
		<h3>New player</h3>
		<h3>{{ ign }}</h3>
		<BigButton @click="confirmNewPlayer()" :label="confirmButtonText" />
	</div>
</template>
